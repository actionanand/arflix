...
@Component({
  ...
  template: `
    ...
    @if (homeResource.error()) {
      ...
      <section class="rail" aria-labelledby="theatres-title">
        <div class="section-heading">
          <h2 id="theatres-title">In Theatres</h2>
        </div>
        <div class="media-grid media-grid--scroll">
          @for (item of homeResource.value().inTheatres; track item.id) {
            <app-media-card [item]="item" />
          }
        </div>
      </section>
      ...
      <section class="rail" aria-labelledby="tv-title">
        <div class="section-heading">
          ...
          <a routerLink="/tv-shows">Browse shows</a>
        </div>
        <div class="media-grid media-grid--scroll">
